# 텀블러 형태·재질 분류 및 설명 시스템

Computer Vision Project · 2026

사용자가 텀블러 이미지를 업로드하면 형태와 재질을 자동으로 분류하고, 해당 종류에 대한 설명을 제공하는 이미지 인식 시스템. **향후 건축 양식 판별로 확장 가능한 구조로 설계.**
...
1. **텀블러 형태·재질 판별** — 이미지 업로드 시 4가지 형태 + 2가지 재질을 자동 분류하고, 종류별 설명(형태 특징, 용도, 관련 정보) 제공
...
```
이미지 업로드 → 전처리(Resize, Normalize) → 모델 추론 → 결과 출력(Confidence) → 설명 출력(형태·재질 설명)
```
...
### 머신비전 구현 방법 — 4갈래 병렬 비교

| 트랙 | 접근 | 형태 분류 | 재질 분류 |
|---|---|---|---|
| OpenCV 룰베이스 | 고전 영상처리 | width profile(폭 프로파일) + `cv2.convexityDefects`(손잡이 검출) | 스펙큘러 하이라이트 등 약한 신호 → 신뢰도 낮음, 한계 인정 |
| Mask R-CNN / Faster R-CNN (DL1) | Instance Segmentation | ResNet50+FPN → Mask 이진화 후 Rule-base와 유사한 기하 특징 추출 | Mask R-CNN으로 ROI 추출 → ResNet50 Material Classification에 입력 (두 모델 결합) |
| ResNet-18, 50 (DL2) | 전이학습 | pretrained + 백본 레이어만 fine-tuning, 18→50 순차 비교 | 동일 |
| EfficientNet | 전이학습 | EfficientDet 적용 학습 | 동일 |
| 3D 모델 (선택) | MV-DUSt3R+ (Meta) | 촬영 사진(4장 내외)으로부터 3D 포인트클라우드/메쉬 생성 | — |

재질 분류의 근본적 어려움: 스테인리스 vs 플라스틱 차이는 표면 질감·하이라이트·반사·국소 색상 패턴 문제라, 무광 스테인리스·유광 플라스틱이 섞이면 Mask/윤곽 기반 접근만으로는 부족 — 권장 방법은 **형태는 세그멘테이션/기하 기반, 재질은 별도 분류기(ResNet50 등)로 역할을 나누는 하이브리드 구조**.
...
| 4 | 설명글 + 파이프라인 통합 + UI — 형태·재질별 설명 작성, Gradio UI 구현 |
...
│   │   └── material_heuristic.py  # 스펙큘러 하이라이트 등
│   │
│   ├── models/
│   │   ├── dl1_maskrcnn/          # Mask/Faster R-CNN (형태+재질 결합)
│   │   ├── dl2_resnet/            # ResNet-18 → 50
│   │   └── efficientnet/
│   │
│   ├── reconstruction_3d/         # (선택) MV-DUSt3R+ 기반 3D 생성
│   │
│   ├── explanation/               # 형태·재질별 설명 텍스트 데이터
│   │
